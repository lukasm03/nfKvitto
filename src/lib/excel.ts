...
export default async function skapaExcelArk(excelInfo: any[]) {
	...
	const läggaTillData = () => {
		...
		excelInfo.forEach((item, i) => {
			const datums = new Date(item.datum.slice(0, 10));
			const priset = Number(item.pris);
			if (item.typavkop === 'Avgift') {
				...
				sheet.getCell(`B${i + 3}`).value = {
					hyperlink: `https://nfdatabas.fly.dev/api/files/kvitton/${item.id}/${item.bild}`,
					text: item.vara
				};
